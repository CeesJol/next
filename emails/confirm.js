export default (token) => {
  return `<div style="background-color: #fafafa; width: 100vw; height: 100vh; margin: 0; margin-top: 30px; padding: 0; position: relative;">
	...
	<p>Please verify your email address to start using Product name.</p>
	<br /> <a href="http://localhost:3000/confirm/${token}"> <button style="background-color: green; border: none; color: white; padding: 10px; border-radius: 2px; width: 100%; cursor: pointer;">Confirm email address</button> </a> <br /><br />
	<p>Or alternatively, copy the link below in your browser:</p>
	<a href="http://localhost:3000/confirm/${token}">localhost:3000/confirm/${token}</a></div>
	</div>
	</div>`;
};
